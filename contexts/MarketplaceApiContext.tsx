...
import {
	...
	GetInstallDataResponse,
	RequestListingOrUpdateRequest,
	RequestListingOrUpdateResponse,
	SearchRequest,
	SearchResponse,
	SetMediaPublicRequest,
	SetMediaPublicResponse,
	UploadFileRequest,
	UploadResponse,
	UploadTextRequest,
} from '../types/slime/MarketplaceApiTypes';
import { Media } from '../types/slime/Media';
import { useSlimeApi } from './SlimeApiContext';
...
type RequestListingOrUpdateCallback = (
	params: RequestListingOrUpdateRequest
) => Promise<RequestListingOrUpdateResponse>;
 ...
 * Provider
 */
export const MarketplaceApiContextProvider = ({ children }: { children: ReactNode | ReactNode[] }) => {
	...
	const requestListingOrUpdate = async (params: RequestListingOrUpdateRequest) => {
		try {
			...
			return {
				...
				message: response.data.message,
			} as RequestListingOrUpdateResponse;
		} catch (e) {
			return {
				currentStatus: 'Error',
				message: 'An unknown error occurred during requestListingOrUpdate',
			} as RequestListingOrUpdateResponse;
		}
	};
	...
	const setMediaPublic = async (params: SetMediaPublicRequest) => {
		try {
			...
			return {
				currentStatus: 'Error',
				message: 'An unknown error occurred during setMediaPublic',
			} as RequestListingOrUpdateResponse;
		}
	};
...
export const useMarketplaceApi = () => {
	...
	if (context === undefined) {
		throw new Error('useMarketplaceApi must be used within a MarketplaceApiContextProvider');
	}
	return context;
};
