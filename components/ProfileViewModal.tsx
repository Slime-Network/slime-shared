            ...
            <Box sx={profileModalStyle}>
                {profile ?
                    <Stack justifyContent="flex-start" direction="row" spacing={2} sx={{ p: 2 }}>
                        ...
                        <Paper elevation={1} sx={{ m: 2, p: 2 }}>
                            <Grid container>
                                {profile?.gostiLinks && JSON.parse(profile.gostiLinks).map((link: SocialLink) => (
                                    <Grid item xs={6} key={link.name}>
                                        <Link href={link.link} target="_blank" rel="noreferrer">{link.name}</Link>
                                    </Grid>
                                ))}
                            </Grid>
                        </Paper>
                    ...
                    :
                    <Stack>
                        <Typography variant="h6">Fetching Profile <CircularProgress /></Typography>
                    </Stack>
                }
            </Box>
        </Modal>
